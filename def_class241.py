# -*- coding: utf-8 -*-
"""
Created on Fri Mar  8 13:58:53 2019
...
########################################################################
mlist = []
mlist.append(Make('m1',('r1','r2','r3','r4'))) #add r3 r4 to represent direct sale
mlist.append(Make('m2',('r1','r2','r3','r4'))) #add r3 r4 to represent direct sale
rlist = []
rlist.append(Retail('r1','b1'))
rlist.append(Retail('r2','b1'))
rlist.append(Retail('r3','b1')) 
rlist.append(Retail('r4','b1')) 
blist = []
blist.append(Buy('b1'))
...
f[1] = 2*(np.sum([t1q[1,:]]))**2 + 2

##############################################################
##############################################################
## edit transaction cost faced by manufacturers here
##############################################################
t1c[0,0] = (2*t1q[0,0] +5)
t1c[0,1] = (2*t1q[0,1] +5)
t1c[0,2] = 0*t1q[0,2]   #tran and handling cost to m
t1c[0,3] = 0*t1q[0,3]   #tran and handling cost to m

t1c[1,0] = (2*t1q[1,0] +5)
t1c[1,1] = (2*t1q[1,1] +5)
t1c[1,2] = 0*t1q[1,2]   #tran and handling cost to m
t1c[1,3] = 0*t1q[1,3]   #tran and handling cost to m
...
c[2] = 0*(np.sum([t1q[:,2]]))**2  #0, have put it like this for diff to work
c[3] = 0*(np.sum([t1q[:,3]]))**2  #0, have put it like this for diff to work
