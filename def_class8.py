# -*- coding: utf-8 -*-
"""
Created on Sun Mar 10 19:57:34 2019
...
mlist.append(Make('m3',('r1','r2')))
rlist = []
rlist.append(Retail('r1',('b1','b2','b3')))
rlist.append(Retail('r2',('b1','b2','b3')))
blist = []
blist.append(Buy('b1'))
blist.append(Buy('b2'))
blist.append(Buy('b3'))
...
f[0] = (2.5*(np.sum([t1q[0,:]]))**2 + 2*(np.sum([t1q[0,:]])) 
+(np.sum([t1q[0,:]]))*(np.sum([t1q[1,:]])))
f[1] = (2.5*(np.sum([t1q[1,:]]))**2 + 2*(np.sum([t1q[1,:]])) 
+(np.sum([t1q[0,:]]))*(np.sum([t1q[1,:]])))
f[2] = (0.5*(np.sum([t1q[2,:]]))**2 + 2*(np.sum([t1q[2,:]])) 
+0.5*(np.sum([t1q[0,:]]))*(np.sum([t1q[2,:]])))

##############################################################
##############################################################
## edit transaction cost faced by manufactuerers here
##############################################################
t1c[0,0] = (0.5*t1q[0,0] +3.5)*t1q[0,0]
t1c[0,1] = (0.5*t1q[0,1] +3.5)*t1q[0,1]
t1c[1,0] = (0.5*t1q[1,0] +3.5)*t1q[1,0]
t1c[1,1] = (0.5*t1q[1,1] +3.5)*t1q[1,1]
t1c[2,0] = (0.5*t1q[2,0] +2)*t1q[2,0]
t1c[2,1] = (0.5*t1q[2,1] +2)*t1q[2,1]
############################################################
############################################################
## edit handling cost of retailers here
############################################################
c[0] = 0.5*(np.sum([t1q[:,0]]))**2
c[1] = 0.5*(np.sum([t1q[:,1]]))**2
############################################################
############################################################
## edit demand functions here    
############################################################
d[0] = -2*rho3[0] - 1.5*rho3[1] + 1000
d[1] = -2*rho3[1] - 1.5*rho3[0] + 1000
d[2] = -2*rho3[2] - 1.5*rho3[0] + 1000
############################################################
############################################################
