# -*- coding: utf-8 -*-
"""
Created on Wed Feb 27 17:03:44 2019
...
f3 = (0*(sum([t1q[0]]))**2 +0.5*(sum([t1q[2]]))**2
                     + 0.5*sum([t1q[0]])*sum([t1q[2]])
                     +0*(sum([t1q[0]]))+2*(sum([t1q[2]])))
f=[f1,f2,f3]
##############################################################
t1c=[]
for i in range(1,m):
...
t1c[2,0] = 0.5*t1q[2,0]**2 +2*t1q[2,0]
t1c[2,1] = 0.5*t1q[2,1]**2 +2*t1q[2,1]
